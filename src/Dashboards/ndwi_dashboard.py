...
def render_aoi_map(
    ...
    st.write(":blue[Area of Interest (AOI)]")
...
# NDWI Line Chart
# --------------------------
def plot_ndwi(df: pd.DataFrame, show_var: bool, width: int = 900, height: int = 460):
    df = df.copy()
    df["date"] = pd.to_datetime(df["date"])
    df = df.sort_values("date")
    if show_var:
...
def ndwi(data, selected_years, show_var, map_view):
    ...
    st.markdown(
        ...
        '>
            💧 Normalized Difference <span style="color: blue;">Water</span> Index ({years_text})
        </h1>
        """,
        unsafe_allow_html=True,
    )
